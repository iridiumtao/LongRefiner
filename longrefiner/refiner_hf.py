...
class LongRefinerHF:
    ...
    def _load_trained_model(
        ...
        self.model = PeftModel.from_pretrained(model, query_analysis_module_lora_path, adapter_name="query_analysis")
        self.model.load_adapter(doc_structuring_module_lora_path, adapter_name="doc_structuring")
        self.model.load_adapter(global_selection_module_lora_path, adapter_name="global_selection")
    ...
    def _generate_batch(self, prompt_list: List[str], lora_name: str, sampling_params: dict):
        self.model.set_adapter(lora_name)
        
        # This is a simplified batching. For production, a more sophisticated batching/padding strategy is needed.
        outputs = []
        for prompt in tqdm(prompt_list, desc=f"Generating with {lora_name}"):
    ...
    def batch_run(
        ...
        # ... (This method remains the same, calling the step-by-step run methods)
    ...
    def run_query_analysis(self, question_list: List[str]) -> List[dict]:
        config = self.step_to_config["query_analysis"]
        ...
        # Simplified generation, one by one. Batching would be more complex to implement here.
        query_analysis_result = []
        for prompt in tqdm(prompt_list, desc="Running Query Analysis"):
            inputs = self.tokenizer(prompt, return_tensors="pt").to(self.device)
            self.model.set_adapter(lora_name)
            with torch.no_grad():
                output = self.model.generate(**inputs, **sampling_params)
    ...
    def run_doc_structuring(self, document_list: List[List[dict]]) -> List[List[dict]]:
        config = self.step_to_config["doc_structuring"]
        prompt_template = config["prompt_template"]
        sampling_params = config["sampling_params"]
        lora_name = config["lora_name"]
        ...
        self.model.set_adapter(lora_name)
        outputs_text = []
        # Simplified generation, one by one. Batching is complex with different lengths.
        for prompt in tqdm(prompt_list, desc="Running Doc Structuring"):
             inputs = self.tokenizer(prompt, return_tensors="pt").to(self.device)
             with torch.no_grad():
                output_ids = self.model.generate(**inputs, **sampling_params)
        ...
        for idx, item_doc_list in enumerate(doc_content_list):
            ...
            for doc_idx, doc_content in enumerate(item_doc_list):
                output_text = outputs_text[start_idx]
    ...
    def run_global_selection(self, question_list: List[str], structured_doc_list: List[List[dict]]) -> List[List[dict]]:
        config = self.step_to_config["global_selection"]
        prompt_template = config["prompt_template"]
        sampling_params = config["sampling_params"]
        lora_name = config["lora_name"]

        prompt_list = []
        # ... (code to build prompt_list is the same as original)
        for question, item_doc_list in zip(question_list, structured_doc_list):
            for doc in item_doc_list:
                ...
                elif isinstance(abstract, list): abstract = "\n".join(abstract)
                title = doc["title"]
        ...
        self.model.set_adapter(lora_name)
        outputs_text = []
        for prompt in tqdm(prompt_list, "Running Global Selection"):
            inputs = self.tokenizer(prompt, return_tensors="pt").to(self.device)
            with torch.no_grad():
                output_ids = self.model.generate(**inputs, **sampling_params)
            output_text = self.tokenizer.decode(output_ids[0], skip_special_tokens=True)
        ...
        for question, item_doc_list in zip(question_list, structured_doc_list):
            ...
            for doc in item_doc_list:
                selected_title = outputs_text[idx]
    ...
    def _collect_hierarchical_nodes(self, question_list: List[str], doc_structuring_result: List[List[dict]] ) -> List[dict]:
        all_nodes = []
                ...
                if "abstract" in doc:
                    if doc["abstract"] is None or doc["abstract"] == "" or doc["abstract"] == []:
                        ...
                        for chunk in doc["abstract"]:
                            all_nodes.append(
                                { "idx": idx, "question": question, "doc_idx": doc_idx, "type": "paragraph", "parent": "abstract", "parent_type": "abstract", "content": chunk, }
                            )
                sections = doc["sections"]
                for section_title, section_dict in list(sections.items()):
                    if section_dict["content"] == [] or section_dict["content"] is None:
                        ...
                        for chunk in section_dict["content"]:
                            all_nodes.append(
                                { "idx": idx, "question": question, "doc_idx": doc_idx, "type": "paragraph", "parent": section_title, "parent_type": "section", "content": chunk, }
                            )
                        if section_dict["subsections"] != {}:
                            for subsection, subsection_content in list(section_dict["subsections"].items()):
                                if subsection_content == "" or subsection_content == None:
                                    ...
                                    for chunk in subsection_content:
                                        all_nodes.append(
                                            { "idx": idx, "question": question, "doc_idx": doc_idx, "type": "paragraph", "parent": subsection, "parent_type": "subsection", "grandparent": section_title, "content": chunk, }
                                        )
        return all_nodes
            ...
            for doc_idx, doc in enumerate(doc_structuring_result[idx]):
                if "abstract" in doc:
                    if doc["abstract"] is None or doc["abstract"] == "":
                        ...
                        node_list.append(
                            { "idx": idx, "question": question, "doc_idx": doc_idx, "type": "abstract", "parent": "root", "parent_type": "root", "title": "abstract", "score": score, }
                        )
                sections = doc["sections"]
                for section_title, section_dict in list(sections.items()):
                    if section_dict["content"] == [] or section_dict["content"] is None:
                        ...
                        if section_dict["subsections"] != {}:
                            for subsection, subsection_content in list(section_dict["subsections"].items()):
                                if subsection_content == "":
                                    ...
                                    node_list.append(
                                        { "idx": idx, "question": question, "doc_idx": doc_idx, "type": "subsection", "parent": section_title, "parent_type": "section", "title": subsection, "score": sub_score, }
                                    )
                    if score_list != []:
                        score = sum(score_list) / len(score_list)
                    else:
                        score = 0
                    node_list.append(
                        { "idx": idx, "question": question, "doc_idx": doc_idx, "type": "section", "parent": "root", "parent_type": "root", "title": section_title, "score": score, }
                    )
